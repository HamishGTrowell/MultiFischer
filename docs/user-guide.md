...
```bash
conda create -n multifischer -c conda-forge python=3.12 numpy pandas matplotlib scipy seaborn pip
conda activate multifischer
...
The editable installation is convenient when working from a clone. Use `python -m pip install .` for a regular installation. `MPLBACKEND=Agg` selects a non-interactive backend suitable for MultiFischer's file-based plots and for headless systems. The module form `python -m multifischer` is equivalent to the installed `multifischer` command.

## 2. Preparing input data

MultiFischer accepts one CSV file per analysis. Rows are spectral sampling points and columns are spectra.
...
Irradiation wavelengths must fall inside the measured spectral range. The requested dark-maximum, metastate-filter, and plotting ranges must each overlap the measured wavelengths. MultiFischer uses the measured row nearest to each irradiation wavelength; interpolation is not applied at this extraction step.
...
All unique irradiation pairs are evaluated at `X = 1`, filtered, and summarized.

> [!NOTE]
> The multipair sensitivity analysis has a known issue. An update is planned for a future release.

If `--out` is omitted, the output directory is named after the input file stem. If the directory already exists, MultiFischer asks whether to reuse it or select another path. Existing files with standard output names may be replaced when an existing directory is reused.
...
| `--sensitivity` | off | Multipair sensitivity option (known issue; update planned). |
...
| `multifischer.log` | Human-readable run log, diagnostics, filter counts, and summary values. |
...
> [!NOTE]
> Multipair sensitivity outputs have a known issue. An update is planned for a future release.
...
Review `pss_matrix.csv` and `hdi_results.csv` rather than relying only on final plots. A small accepted count or rejection of most pairs indicates that the reported mean is fragile for that irradiation wavelength.
...
Archive the input CSV, MultiFischer version, complete output directory, and `run_settings.json`. Report the irradiation wavelengths, all non-default ranges and filters, accepted counts, and final mean and sample standard deviation.
